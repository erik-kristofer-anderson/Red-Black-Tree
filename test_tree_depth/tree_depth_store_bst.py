...
for num_elements in range(num_elements_start, num_elements_stop, num_elements_step):
    ...
    for _ in range(repetitions):
        ...
        for i in range(num_elements):
            ...
            bst_1.add_node_by_value(x)
        y = bst_1.find_tree_depth()
...
if save:
    with open(f"data_bst/depth_results_{datetime.datetime.now()}_metadata.txt", 'w') as file:
        s = ""
    ...
    with open(f"data_bst/depth_results_{datetime.datetime.now()}.txt", 'w') as file:
...
plt.title("Tree Height vs Number of Elements in Binary Search Tree")
...
if save:
    plt.savefig(f"data_bst/depth_results_{datetime.datetime.now()}_plot.pdf")
if show_plot:
    plt.show()
